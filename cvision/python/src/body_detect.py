...
def detect(img, cascade):
    rects = cascade.detectMultiScale(img, scaleFactor=2, minNeighbors=2, minSize=(30, 30), flags=cv2.CASCADE_SCALE_IMAGE)
    if len(rects) == 0:
        return []
    rects[:,2:] += rects[:,:2]
    return rects
...
def main():
    ...
    cascade = cv2.CascadeClassifier("../data/banana_classifier.xml")
